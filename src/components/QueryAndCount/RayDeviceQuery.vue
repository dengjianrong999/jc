			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">射线装置类别</th>
						<th scope="col">射线装置数量</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td :title="item.deviceCategory">{{item.deviceCategory}}</td>
						<td :title="item.deviceNumber">{{item.deviceNumber}}</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				let temData = {
					...
					deviceName: this.zsgcValue2,
					deviceCategory: this.zsgcValue3,
				...
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}radialdevice/listPageJson`,
							params: temData
						})
						.then(function(res) {
