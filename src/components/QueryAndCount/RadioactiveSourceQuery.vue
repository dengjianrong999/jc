	...
	<div class="fs-right">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<el-select placeholder="--请选择类型--" v-model="zsgcValue4">
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td width="100px" :title="item.category">{{item.category}}</td>
						<td width="70px" :title="item.radiatiotType">{{item.radiatiotType}}</td>
						<td width="100px" :title="item.totalApprovedActivity">{{item.totalApprovedActivity}}</td>
						<td width="100px" :title="item.activity">{{item.activity}}</td>
						<td width="100px" :title="item.totalNumber">{{item.totalNumber}}</td>
		...
		methods: {
			...
			SeeOrModify(id, num) {
				...
				let layerIndex = layer.open({
					...
					area: ['75%', '70%'],
					content: '#/RadioactiveSourceQueryWD/' + id,
